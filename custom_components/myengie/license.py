"""
Modul de licențiere pentru integrarea MyENGIE România.
...
- Câmpul `integration` identifică integrarea (fleet, eonmyline, myengie, etc.)
...
class LicenseManager:
    """Gestionează licența pentru integrarea MyENGIE România.
    ...
    async def async_load(self) -> None:
        ...
        _LOGGER.debug("[MyENGIE:License] Încep async_load()")
        try:
            stored = await self._store.async_load()
            self._data = dict(stored) if stored else {}
        except Exception:  # noqa: BLE001
            _LOGGER.warning(
                "[MyENGIE:License] Storage corupt sau ilizibil "
                "— pornesc cu date goale (serverul va restaura starea)"
            )
            self._data = {}
        _LOGGER.debug(
            "[MyENGIE:License] Date din storage: %d chei (%s)",
            len(self._data),
        ...
        _LOGGER.debug(
            "[MyENGIE:License] Fingerprint generat: %s... (hw: %s...)",
        ...
        if self._status_token:
            ...
            _LOGGER.debug(
                "[MyENGIE:License] Cache restaurat: status=%s, cache_valid=%s",
                cached_status,
                cache_valid,
            )
        else:
            _LOGGER.debug("[MyENGIE:License] Niciun cache de status — prima rulare")

        # Verifică statusul la server (prima verificare la startup)
        _LOGGER.debug("[MyENGIE:License] Verific statusul la server (startup)...")
        await self.async_check_status()

        self._loaded = True
        final_status = self.status
        _LOGGER.debug(
            "[MyENGIE:License] async_load() finalizat — status=%s, is_valid=%s",
            final_status,
            self.is_valid,
        )
        ...
        if final_status == "licensed":
            ...
            _LOGGER.info(
                "[MyENGIE:License] ✓ Licență ACTIVĂ (cheie: %s)", key
            )
        elif final_status == "trial":
            days = self.trial_days_remaining
            _LOGGER.info(
                "[MyENGIE:License] ⏳ Perioadă de evaluare (trial): "
                "%d zile rămase", days
            )
        elif final_status == "expired":
            _LOGGER.warning(
                "[MyENGIE:License] ✗ EXPIRAT — perioada de evaluare "
                "sau licența a expirat. Senzorii nu vor funcționa."
            )
        else:
            _LOGGER.warning(
                "[MyENGIE:License] ✗ FĂRĂ LICENȚĂ (status=%s) — "
                "senzorii nu vor funcționa.", final_status
            )

    async def _async_save(self) -> None:
        """Salvează datele de licență."""
        _LOGGER.debug("[MyENGIE:License] Salvez datele în storage")
    ...
    def _generate_fingerprint(self) -> str:
        ...
        - Salt: face fingerprint-ul specific integrării MyENGIE România
        """
        componente: list[str] = []
    ...
    async def async_check_status(self) -> dict[str, Any]:
        ...
        if self._is_status_cache_valid():
            _LOGGER.debug(
                "[MyENGIE:License] Cache valid — folosesc token existent "
        ...
        _LOGGER.debug(
            "[MyENGIE:License] Cache expirat sau inexistent — "
        ...
        try:
            ...
            async with session.post(
                ...
                headers={
                    ...
                    "User-Agent": "MyEngie-HA-Integration/1.0",
                },
            ) as resp:
                _LOGGER.debug(
                    "[MyENGIE:License] Server /check răspuns: HTTP %d",
                    resp.status,
                )
                ...
                if resp.status == 200 and "status" in result:
                    ...
                    if not self._verify_token_signature(result):
                        _LOGGER.warning(
                            "[MyENGIE:License] Semnătura token-ului de status "
                    ...
                    if server_key and self._data.get("license_key") != server_key:
                        ...
                        _LOGGER.debug(
                            "[MyENGIE:License] license_key sincronizat "
                    ...
                    _LOGGER.debug(
                        "[MyENGIE:License] Status actualizat de la server — %s "
                        "(valid_until: %s)",
                        server_status,
                    ...
                    if server_status == "expired":
                        _LOGGER.warning(
                            "[MyENGIE:License] Server confirmă: EXPIRAT "
                            "(trial_days_remaining=0)"
                        )
                    elif server_status == "trial":
                        _LOGGER.info(
                            "[MyENGIE:License] Server confirmă: TRIAL "
                            "(zile rămase: %s)",
                    ...
                    if (
                        ...
                        _LOGGER.warning(
                            "[MyENGIE:License] Licență expirată "
                ...
                if result.get("error") == "invalid_hmac":
                    if self._data.get("client_secret") and not self._hmac_retry_done:
                        _LOGGER.warning(
                            "[MyENGIE:License] HMAC invalid — client_secret "
                    ...
                    _LOGGER.error(
                        "[MyENGIE:License] HMAC invalid (retry epuizat). "
                        "Serverul nu recunoaște acest dispozitiv."
                    )
                else:
                    _LOGGER.warning(
                        "[MyENGIE:License] răspuns invalid de la /check — %s",
                        result,
                    )
                return self._status_token
            ...
            _LOGGER.warning(
                "[MyENGIE:License] eroare de rețea la verificare status "
                "(încercare #%d) — %s",
            ...
            self._consecutive_failures += 1
            _LOGGER.warning(
                "[MyENGIE:License] eroare neașteptată la verificare status "
                "(încercare #%d) — %s",
                self._consecutive_failures,
                err,
            )
    ...
    def is_licensed(self) -> bool:
        ...
        if not self._verify_token_signature(token):
            _LOGGER.warning("[MyENGIE:License] semnătură token activare invalidă")
            return False

        # Verifică fingerprint-ul din token-ul de activare
        if token.get("fingerprint") != self._fingerprint:
            _LOGGER.warning("[MyENGIE:License] fingerprint token nu se potrivește")
            return False

        # Verifică expirarea din token-ul de activare (licențe anuale)
        expires_at = token.get("expires_at")
        if expires_at and time.time() > expires_at:
            _LOGGER.info("[MyENGIE:License] licența a expirat (token local)")
            return False
        ...
        if self._status_token and self._is_status_cache_valid():
            ...
            if server_status not in ("licensed", "trial"):
                _LOGGER.warning(
                    "[MyENGIE:License] serverul raportează status '%s' — licență invalidă",
                    server_status,
                )
                return False
        ...
        if self._status_token and not self._is_status_cache_valid():
            if self._is_within_grace_period():
                if not self._cache_expiry_warned:
                    ...
                    _LOGGER.warning(
                        "[MyENGIE:License] cache expirat — funcționare în perioadă "
                ...
                if not self._cache_expiry_warned:
                    _LOGGER.warning(
                        "[MyENGIE:License] cache expirat + perioadă de grație depășită "
    ...
    def license_key_masked(self) -> str | None:
        """Returnează cheia de licență mascată (ex: ENGIE-XXXX-****)."""
        key = self._data.get("license_key")
        ...
        try:
            ...
            async with session.post(
                f"{LICENSE_API_URL}/validate",
                    ...
                    _LOGGER.warning(
                        "[MyENGIE:License] heartbeat respins — %s",
                        result.get("error", "necunoscut"),
                    )
                    return False

        except Exception:  # noqa: BLE001
            _LOGGER.debug("[MyENGIE:License] heartbeat eșuat (rețea indisponibilă)")
            return False
        ...
        try:
            ...
            async with session.post(
                f"{LICENSE_API_URL}/activate",
                    ...
                    _LOGGER.debug(
                        "[MyENGIE:License] /activate răspuns: HTTP %d",
                        resp.status,
                    )
                    ...
                    if resp.status != 200:
                        ...
                        _LOGGER.warning(
                            "[MyENGIE:License] activare eșuată — "
                            "HTTP %d: %s",
                            resp.status,
                            body[:500],
                        )
                    ...
                    if result.get("success"):
                        ...
                        _LOGGER.info(
                            "[MyENGIE:License] licență activată cu succes (%s)",
                            token.get("license_type", "necunoscut"),
                        )

                        # Auto-reload: reîncarcă toate entry-urile myengie
                    ...
                    _LOGGER.warning(
                        "[MyENGIE:License] activare eșuată — %s (răspuns: %s)",
                        error,
                        result,
                    )
            ...
            _LOGGER.error(
                "[MyENGIE:License] eroare de rețea la activare — %s", err
            )
            return {"success": False, "error": "network_error"}
        except Exception as err:  # noqa: BLE001
            _LOGGER.error(
                "[MyENGIE:License] eroare neașteptată la activare — %s", err
            )
    ...
    async def async_deactivate(self) -> dict[str, Any]:
        ...
        try:
            ...
            async with session.post(
                f"{LICENSE_API_URL}/deactivate",
                    ...
                    if resp.status == 200 and result.get("success"):
                        ...
                        _LOGGER.info(
                            "[MyENGIE:License] licență dezactivată cu succes"
                        )
            ...
            _LOGGER.error("[MyENGIE:License] eroare la dezactivare — %s", err)
            return {"success": False, "error": "network_error"}
                ...
                f"{LICENSE_API_URL}/notify",
                    ...
                    if resp.status == 200:
                        ...
                        if not result.get("success"):
                            _LOGGER.warning(
                                "[MyENGIE:License] Server a refuzat '%s': %s",
                                action, result.get("error"),
                            )
                    else:
                        _LOGGER.warning(
                            "[MyENGIE:License] Notify HTTP %d pentru '%s'",
                            resp.status, action,
                        )
        except Exception as err:  # noqa: BLE001
            _LOGGER.debug(
                "[MyENGIE:License] Nu s-a putut raporta '%s': %s",
                action, err,
            )
    ...
    async def _async_reload_entries(self) -> None:
        """Reîncarcă toate entry-urile myengie după activare/dezactivare.
        ...
        _LOGGER.info(
            "[MyENGIE:License] Reîncarc %d entry-uri după schimbarea licenței",
            len(entries),
        )
    ...
    def _verify_token_signature(self, token: dict[str, Any]) -> bool:
        ...
        try:
            ...
            _LOGGER.warning(
                "[MyENGIE:License] nicio cheie publică nu a validat semnătura"
            )
            return False

        except Exception as err:  # noqa: BLE001
            _LOGGER.debug(
                "[MyENGIE:License] verificare semnătură eșuată — %s", err
            )
            return False
